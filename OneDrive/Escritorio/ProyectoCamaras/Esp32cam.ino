...
const char* ssid = "INFINITUM0114";   //--> Enter your SSID / your WiFi network name.
const char* password = "4bzTi4cYKB"; //--> Enter your WiFi password.
/* ======================================== */

/* ======================================== Variables for telegram bot tokens. */
String BOTtoken = "5409549436:AAGxU-4GDUjuyiwKHLUhk2BgxweS01RvJl4"; //--> your Bot Token (Get from Botfather).
/* ======================================== */

/* ======================================== @myidbot ID */
// Use @myidbot to find out the chat ID of an individual or a group.
// Also note that you need to click "start" on a bot before it can message you.
String CHAT_ID = "-701892242";
/* ======================================== */
...
void FB_MSG_is_photo_send_successfully (bool state) {
  ...
  if(state == false) {
    send_feedback_message += "Desde MedicamentosPeligrosos :\n\n";
    send_feedback_message += "ESP32-CAM fallo al enviar foto.\n";
    send_feedback_message += "Sugerencia :\n";
    send_feedback_message += "- Resetea la ESP32-CAM\n";
    ...
    send_feedback_message += "/start@Centinela02Bot : to see all commands.";
    bot.sendMessage(CHAT_ID, send_feedback_message, "");
  } else {
    Serial.println("La foto se envio correctamente.");
    send_feedback_message += "MedicamentosPeligrosos :\n\n";
    send_feedback_message += "Foto enviada satisfactoriamente.\n\n";
    send_feedback_message += "/start@Centinela02Bot : para ver todos los comandos."; // Aquí quiza este el cambio
    bot.sendMessage(CHAT_ID, send_feedback_message, ""); 
  }
}
/* ________________________________________________________________________________ */
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start@Centinela02Bot") {
      send_feedback_message += "MedicamentosPeligrosos :\n\n";
      send_feedback_message += "Bienvenido , " + from_name + "\n";
      send_feedback_message += "Usa los siguientes comandos para interactuar con la MedicamentosPeligrosos \n";
      send_feedback_message += "/Capturar_foto@Centinela02Bot : Para tomar una nueva foto\n";
      send_feedback_message += "/Capturar_foto_con_LED@Centinela02Bot : Para tomar una nueva foto con LED FLash\n";
      send_feedback_message += "/Encender_LED@Centinela02Bot : Encender LED \n";
      send_feedback_message += "/Apagar_LED@Centinela02Bot : Apagar LED  \n";
      bot.sendMessage(CHAT_ID, send_feedback_message, "");
      Serial.println("------------");
    }
    
    // The condition if the command received is "/flash_on".
    if (text == "/Encender_LED@Centinela02Bot") {
      LEDFlash_State(ON);
      Serial.println("LED Flash On");
      Serial.println("------------");
      send_feedback_message += "MedicamentosPeligrosos :\n\n";
      send_feedback_message += "LED Flash encendido\n\n";
      send_feedback_message += "/start@Encender_LED@Centinela02Bot : Para ver todos lo comandos.";
      bot.sendMessage(CHAT_ID, send_feedback_message, "");
    }
    
    // The condition if the command received is "/flash_off".
    if (text == "/Apagar_LED@Centinela01Bot") {
      LEDFlash_State(OFF);
      Serial.println("LED Flash apagado");
      Serial.println("------------");
      send_feedback_message += "MedicamentosPeligrosos :\n\n";
      send_feedback_message += "LED Flash apagado\n\n";
      send_feedback_message += "/start@Centinela02Bot : Para ver todos lo comandos.";
      bot.sendMessage(CHAT_ID, send_feedback_message, "");
    }
    
    // The condition if the command received is "/capture_photo".
    if (text == "/Capturar_foto@Centinela02Bot") {
      sendPhoto = true;
      Serial.println("New photo request");
    }
    
    // The condition if the command received is "/capture_photo_with_LEDFlash".
    if (text == "/Capturar_foto_con_LED@Centinela02Bot") {
